...
from envtether.models.findings import Finding, FindingCategory, Recommendation, Severity
...
class HealthScoreEngine:
    ...
    def compute(
        ...
        drift_findings = [f for f in findings if f.category == FindingCategory.CONFIGURATION_DRIFT]
        ...
        # 8. Secret hygiene
        secret_findings = [f for f in findings if f.category == FindingCategory.EXPOSED_SECRET]
    ...
    def _check_naming(variables: list[ConfigVariable]) -> int:
        ...
        for var in variables:
            name = var.name
            if name != name.upper() or not all(c.isalnum() or c == "_" for c in name):
                count += 1
        return count
